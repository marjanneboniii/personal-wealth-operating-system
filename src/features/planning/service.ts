  ...
  return db.transaction(async (tx) => {
    ...
    if (!contraAccountId) {
      ...
      if (!contraAccountId) {
        ...
        throw new Error(
          receivable
            ? "سرفصل «دریافت مطالبات» برای شما ساخته نشد؛ در «تنظیمات ← حساب‌ها» یک حساب درآمد بسازید."
            : "سرفصل «پرداخت اقساط» برای شما ساخته نشد؛ در «تنظیمات ← حساب‌ها» یک حساب هزینه بسازید.",
        );
      }
    }
