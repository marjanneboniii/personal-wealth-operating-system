 ...
 *   • Name  → «سرای» — one short, memorable, creative word, generated by the
 *             system. Location/type are NEVER repeated in the name; they stay
 *             in the structured master-data columns (type/city/neighborhood).
 *   • Symbol→ the shortest canonical global RWA id (`1`, `2`, …), generated,
 *             unique (DB unique constraint on assets.symbol) and immutable.
...
/**
 * The single short, creative display name for every registered property.
 *
 * Deliberately NOT a single letter («ت» / «م») and NOT the long concatenation
 * «آپارتمان — اهواز — شهرک دانشگاه»: the type, city and neighborhood are
 * already stored in their structured master-data columns, so repeating them
 * in the name only makes every screen noisy. «سرای» is four letters, poetic,
 * unambiguous and shown identically across the whole app.
 */
export const REAL_ESTATE_ASSET_NAME = "سرای";

/**
 * Number of already-registered properties with the same
 * (city, neighborhood, property type) — informational only. The compact
 * symbol uses the separate global RWA sequence.
 */
...
/** «سرای» — always. The short symbol (`1`, `2`, …) is the unique discriminator. */
export function buildAssetName(): string {
  return REAL_ESTATE_ASSET_NAME;
}

/** Shortest canonical RWA id (`1`, `2`, …). Shared with vehicles. */
