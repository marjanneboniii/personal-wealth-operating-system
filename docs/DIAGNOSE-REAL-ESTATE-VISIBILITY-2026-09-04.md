...
| ۳ | `assets.deleted_at IS NOT NULL` | ملک حذف/فروش شده یا `repairOrphanedRealEstate()` آن را بی‌اثر کرده. `deleteRealEstateAsset` و `sellRealEstateAsset` ردیف ملک را حذف و دارایی را soft-delete می‌کنند؛ نماد تاریخی روی همان ردیف حفظ می‌شود و فقط در ایندکس یکتای فعال‌ها شرکت نمی‌کند | سند دفترکل و تاریخچه دست‌نخورده‌اند؛ بازگردانی یعنی `deleted_at = NULL` + بررسی اینکه شناسه کوتاه آزادشده (`۰۰۱`…) به دارایی دیگری نچسبیده باشد |
...
* **فهرست خالی است و کلیک روی «املاک من» خطای سراسری می‌دهد، در حالی که دارایی در `/assets` شمرده می‌شود** → علتِ داده‌ای نیست؛ نگاشتِ props صفحه بود
  (یک جابه‌جاییِ یک‌خانه‌ای در `Promise.all` که لیست املاک را با خلاصهٔ پرتفوی عوض می‌کرد).
  رفع در [`FIX-2026-09-04-asset-registry-prop-misalignment.md`](./FIX-2026-09-04-asset-registry-prop-misalignment.md)؛
  از این نسخه به بعد مدل نمایش با **نام** بارگذاری می‌شود.
